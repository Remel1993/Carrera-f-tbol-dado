...
const TabButton = ({ active, onClick, icon, label }) => (
  <button
    onClick={onClick}
    className={`min-w-[62px] px-2.5 py-2 flex flex-col items-center justify-center gap-1 rounded-[14px] shrink-0 transition-all ${
...
const ConfirmSignModal = ({ title, teamName, detail, note, onCancel, onConfirm }) => (
  <div className='fixed inset-0 z-[80] bg-black/85 backdrop-blur-sm flex items-center justify-center p-6'>
    <motion.div initial={{ scale: 0.92, y: 16, opacity: 0 }} animate={{ scale: 1, y: 0, opacity: 1 }} exit={{ opacity: 0 }}
      className='w-full max-w-xs bg-gradient-to-b from-slate-900 to-slate-950 rounded-[1.75rem] border border-amber-500/40 p-6 text-center shadow-2xl'>
...
export const CareerView = ({
  ...
  const coreMet = coreObjectives.filter(o => o.done).length;
        ...
        <motion.div key={tab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className='flex-grow space-y-4'>
          {tab === 'main' && (
            <>
              ...
              {divisionFinished ? (
                <Panel className='p-5 text-center relative overflow-hidden'>
                  ...
                  <Trophy size={32} className='text-yellow-400 mx-auto mb-3 animate-bounce' />
                  ...
                  <div className='grid grid-cols-2 gap-2 mt-4'>
                    <button
                      ...
                      className='w-full bg-gradient-to-r from-amber-500 to-yellow-600 text-slate-950 py-3.5 rounded-2xl text-[10px] font-black uppercase italic tracking-widest active:scale-95 transition-all shadow-md'
                    >
                      Buzón de Ofertas {career.offers?.length > 0 && `(${career.offers.length})`}
                    </button>
                    <button
                      onClick={onOpenReview}
                      disabled={reviewDone || contractSigned}
                      className='w-full bg-slate-800 hover:bg-slate-700 text-white py-3.5 rounded-2xl text-[10px] font-black uppercase italic tracking-widest active:scale-95 transition-all disabled:opacity-30 border border-white/10'
                    >
                      {contractSigned ? 'Balance cerrado' : reviewDone ? 'Balance resuelto' : 'Ver balance'}
                    </button>
                  </div>
                </Panel>
                    ...
                    <div className='min-w-0'>
                      <div className='flex items-center gap-1.5 flex-wrap'>
                        ...
                        {career.medicalImmunityWeeks > 0 && (
                          <span className='text-[8px] font-black uppercase px-1.5 py-0.2 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 flex items-center gap-0.5'>
                            <HeartPulse size={9} /> {career.medicalImmunityWeeks} sem.
                          </span>
                        )}
                      </div>
          ...
          {tab === 'squad' && (
            <Panel className='p-5 space-y-4'>
              <div className='flex items-center justify-between'>
                <div>
                  <p className='text-[9px] font-black uppercase tracking-widest text-emerald-400'>
                    Centro de Desarrollo · {career.pe} PE Disponibles
                  </p>
              ...
              >
                <Zap size={16} /> Abrir Ventana de Entrenamiento (Steppers / Asignar PE)
              </button>
              ...
              <div className='bg-black/30 rounded-2xl p-4 border border-white/5 flex items-center justify-between gap-3'>
                <div>
                  <p className='text-[10px] font-black uppercase italic text-white flex items-center gap-1.5'>
                    <Dices size={14} className='text-amber-400' /> Entrenamiento Semanal (1D6)
                  </p>
                  <p className='text-[9px] font-bold text-slate-400 mt-0.5'>
                    {hasTrainedThisMatchday
                      ? 'Ya realizaste la sesión de entrenamiento para esta jornada.'
                      : 'Lanza 1D6: gana +7 o +5 PE, o arriesga con lesión y fisio.'}
                  </p>
                </div>
                <button
                  onClick={() => setShowDrillModal(true)}
              ...
              {career.medicalImmunityWeeks > 0 && (
                <div className='bg-emerald-950/40 border border-emerald-500/30 rounded-2xl p-3.5 flex items-center gap-3'>
                  ...
                  <div className='text-[9px] font-bold text-emerald-200'>
                    ...
                    Protegido contra lesiones durante {career.medicalImmunityWeeks} jornada{career.medicalImmunityWeeks > 1 ? 's' : ''} más tras Fisioterapia de Élite.
                  </div>
                </div>
              )}

              {maxed ? (
                <div className='bg-emerald-500/10 border border-emerald-400/30 rounded-2xl px-4 py-3 flex items-start gap-3'>
                ...
                {[
                  ...
                  return (
                    <div key={a.key} className='flex items-center gap-3 bg-black/30 rounded-2xl px-4 py-3 border border-white/5'>
                      ...
                      >
                        {capped ? 'Techo' : `Gestionar · ${cost} PE`}
                      </button>
                    </div>
                  );
                })}
                            ...
                            >
                              ...
                              <div className='flex items-center justify-between gap-2'>
                                <div className='flex items-center gap-2.5 min-w-0'>
                                  ...
                                  />
                                  <div className='min-w-0'>
                                    <p className='text-[8px] font-bold text-slate-400 uppercase'>vs Rival</p>
                                ...
                                <div className='shrink-0 text-right'>
                                  {item.played ? (
                                    ...
                                    <span className='text-[8px] font-black uppercase px-2 py-1 rounded-xl bg-slate-800/80 text-sky-300 border border-sky-500/20'>
                                      Por disputar
                                    </span>
                                  )}
                                </div>
                        ...
                        >
                          ...
                          <div className='bg-black/40 rounded-xl p-3 border border-white/5 space-y-1.5'>
                            ...
                            <div className='flex items-center justify-between pt-1 border-t border-white/5 text-[8px] font-bold'>
                              <span className='text-slate-400 uppercase'>Objetivo Exigido:</span>
...
export const CareerSeasonReviewModal = ({ review, onAcceptOffer, onRenew, onStay, ui }) => {
  ...
  return (
    <div className='fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4'>
      <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className='w-full max-w-sm bg-gradient-to-b from-slate-900 to-slate-950 rounded-[2rem] border border-amber-500/30 overflow-hidden max-h-[88vh] overflow-y-auto custom-scrollbar'>
