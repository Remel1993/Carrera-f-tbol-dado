  ...
  compIds.forEach(compId => {
    ...
    [1, 2].forEach(div => {
      ...
      sorted.forEach((team, idx) => {
        ...
        // Medir similitud de Tier y Posición
        const tierDiff = Math.abs(tier - currentTier);
        const posDiff = Math.abs(pos - currentPos);
        const similarityScore = tierDiff * 8 + posDiff;
...
export const generateRumors = (comps = {}, career = {}) => {
  ...
  compIds.forEach(compId => {
    ...
    [1, 2].forEach(div => {
      ...
      sorted.forEach((t, idx) => {
        if (t.id === career?.teamId) return; // No generar rumor del propio usuario salvo si aplica
        const tier = tierOf(t);
        const inRelegation = idx >= total - 3;
        const heavyLosses = (t.l || 0) >= 3 && (t.w || 0) <= 1 && (t.p || 0) >= 4;

        if ((inRelegation || heavyLosses) && tier >= 2 && rumors.length < 5) {
          rumors.push({
            id: `rumor-danger-${t.id}`,
            type: 'danger',
            tag: 'RUMOR',
      ...
      const needyClub = sorted.find(t => (t.p || 0) >= 6 && t.pts <= 4 && t.id !== career?.teamId);
      if (needyClub && rumors.length < 7) {
        rumors.push({
          id: `rumor-vacancy-${needyClub.id}`,
          type: 'vacancy',
          tag: 'VACANTE',
    ...
    if (comp.name && rumors.length < 9) {
      rumors.push({
        id: `rumor-press-${compId}`,
        type: 'press',
        tag: 'PRENSA',
  ...
  if (rumors.length === 0) {
    rumors.push(
      { id: 'r-1', type: 'press', tag: 'PRENSA', text: 'El mercado de directores técnicos se agita de cara a la próxima ventana de fichajes.' },
      { id: 'r-2', type: 'rumor', tag: 'RUMOR', text: 'Varios clubes de primera división envían ojeadores para monitorear tácticas emergentes.' },
      { id: 'r-3', type: 'press', tag: 'MERCADO', text: 'Los representantes de entrenadores buscan proyectos ambiciosos en las principales ligas.' }
    );
  }
